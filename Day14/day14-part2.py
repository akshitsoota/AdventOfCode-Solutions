...
def main():
    ...
    state = [[0, 1, 0, 0] for ctr in range(len(reindeers))]
    # We keep state as: total distance covered, flying (1) or sleeping (2) and time spent doing any activity, points accumulated
    for time in range(total_time):
        ...
        # Time to calculate who gets the points
        max_at_this_point = max([x[0] for x in state])
        for ctr in range(len(reindeers)):
            # Check if we match the max distance
            if state[ctr][0] == max_at_this_point:
                state[ctr][3] += 1
    # Print out the final stats
    print max([x[3] for x in state])
